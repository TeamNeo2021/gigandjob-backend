...
export class CandidateSchedulerService implements CandidateScheduler {
    constructor(
        @Inject(forwardRef(() => 'CandidateApplicationService')) private candidateApplicationService: CandidateApplicationService,
    ...
    async scheduleCandidateReactivation(id: string, at: Date): Promise<void> {
        ...
        const job = new CronJob({
            ...
            onTick: async () => {
                try {
                    await this.candidateApplicationService.Handle(new ReactivateCandidateDTO(id))
                } catch (e) {
                    console.error(e)
                }
            },
