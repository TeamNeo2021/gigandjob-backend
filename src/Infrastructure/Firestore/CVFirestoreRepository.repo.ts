...
function entityToClass(cvResult: CVEntity) {
  ...
  return Cv.submitCv(
    ...
    new CvCandidate(
      cvResult.candidate.id,
      new Date(cvResult.candidate.birthdate),
    ),
...
export class CVFirestoreRepository implements ICVCommandRepository {
  constructor(
    @Inject('candidates') private repo: CollectionReference<CVEntity>,
  ) {}
  ...
  async save(cv: Cv): Promise<void> {
    ...
    const cvSave = await this.repo.doc(cv.id.id).set({
      id: cv.id.id,
      candidate: {
        id: cv.candidate.id,
