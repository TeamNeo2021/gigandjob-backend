...
export class OfferFirestoreAdapter implements IOfferRepository{
    constructor(@Inject('offers') private offerRepository: CollectionReference<OfferDTO>) {}
    ...
    async getOfferById(id_offer: string): Promise<OfferDTO> {
        const offerQuery = await this.offerRepository.where('id', '==', id_offer).get(),
    ...
    async likeOffer(data: LikeOfferDTO) {
        try {
            ...
            const userLike = await this.offerRepository.doc(data.id_offer).collection('likes').doc(data.id_candidate).get();
           let likeOffer = userLike.data().isLiked ? false : true; //Si ya le dio like ahora se desactiva si no le ha dado like entonces le da like
            ...
            let result = await this.offerRepository.doc(data.id_offer).collection('likes').doc(data.id_candidate).set({
