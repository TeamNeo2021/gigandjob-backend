...
function entityToClass(candidateResult: CandidateEntity) {
  return new Candidate(
    ...
    new CandidateStateVo(
      candidateResult.state.value,
      candidateResult.state.suspensionCount,
    ),
    new CandidateFullNameVo(
      candidateResult.name.names,
      candidateResult.name.lastnames,
    ),
    new CandidatePhoneVo(
      candidateResult.phone.areaCode,
      candidateResult.phone.phoneNumber,
    ),
    ...
    new CandidateLocationVo(
      candidateResult.location.latitude,
      candidateResult.location.longitude,
    ),
  );
}
...
export class CandidateFirestoreAdapter implements ICandidateRepository {
  constructor(
    @Inject('candidates') private repo: CollectionReference<CandidateEntity>,
  ) {}
