...
class ApplicationFirestoreAdapter implements IApplicationRepository {
  //todo Delete this, this goes in OfferFirestoreAdapter
  constructor(
    @Inject('applications')
    private collection: CollectionReference<ApplyToOfferDTO>,
  ) {}
  ...
  async cancelApplication(application_id: string) {
    try {
      const result = await this.collection.doc(application_id).update({
  ...
  async findAll(): Promise<ApplyToOfferDTO[]> {
    try {
      ...
      const applicationsList = result.docs.map(
        (doc) => new ApplyToOfferDTO(doc.data()),
      );
      console.log(
        'MeeetingFirestoreAdapter: findAll applicationsList: ',
        applicationsList,
      );
