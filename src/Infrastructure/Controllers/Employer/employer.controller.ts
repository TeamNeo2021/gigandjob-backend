...
export class EmployerController {
  ...
  constructor(
    @Inject('EmployerApplicationService')
    private employerService: EmployerApplicationService,
  ) {}
  ...
  ReactivateEmployer(@Body('id') id: string): any {
    const request: ReactivateEmployerDTO = new ReactivateEmployerDTO(id);
  ...
  EliminateEmployer(@Body('id') id: string): any {
    const request: EliminateEmployerDTO = new EliminateEmployerDTO(id);
    this.employerService.Handle(request);
