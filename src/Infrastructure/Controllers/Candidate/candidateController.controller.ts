...
export class CandidateController {
  constructor(
    ...
    private service: CandidateApplicationService,
  ) {}
  ...
  @Post(':id/suspend')
  async suspendCandidate(
    @Param('id') candidateId: string,
    @Body() suspensionBody: CaniddateSuspensionBody,
  ) {
    await this.service.Handle(
      new SuspendCandidateDTO(
        candidateId,
        new Date(Date.parse(suspensionBody.until)),
      ),
    );
