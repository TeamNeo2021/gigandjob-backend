...
export class OfferReportVO {
    private constructor(public readonly reporterId: string, public readonly reason: string) {}

    static Create(reporterId: string, reason: string) {
        if (!reporterId || reporterId.trim() == "" || !reporterId.match(UUID_FORMAT) || reporterId.match(UUID_FORMAT).length == 0) {
