...
export class Cv<State extends CvState = CvState> extends AggregateRoot {
  ...
  protected EnsureValidState(): void {
    if (this.studies.length == 0)
      throw new InvalidCVStudiesError('Studies cannot be empty');
    if (this.workExperiences.length == 0)
      throw new InvalidCvWorkExperienceError('Studies cannot be empty');

    this.studies.forEach((study) => {
      if (
        this.candidate.birthdate >= study.startDate ||
        this.candidate.birthdate >= study.finishDate
      )
        throw new InvalidCVStudiesError(
          'The studies start date nor finish date cannot be before the birthdate',
        );
    });
    this.workExperiences.forEach((workExperience) => {
      if (
        this.candidate.birthdate >= workExperience.startDate ||
        this.candidate.birthdate >= workExperience.finishDate
      )
        throw new InvalidCvWorkExperienceError(
          'The work experience start date nor finish date cannot be before the birthdate',
        );
    });
  }
  ...
  approve(this: Cv<CvState.Submitted>) {
    const event = new CvApprovedDomainEvent(this.id);
    const approvedCv = new Cv(
      this.description,
  ...
  reject(this: Cv<CvState.Submitted>) {
    const event = new CvRejectedDomainEvent(this.id);
    const approvedCv = new Cv(
      this.description,
      this.workExperiences,
      this.studies,
