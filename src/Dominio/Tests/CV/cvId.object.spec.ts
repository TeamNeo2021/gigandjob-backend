...
describe('CV identity', () => {
  ...
  it('should fail when invalid id is used', () => {
    ...
    expect(() => new CvId('944fadf6603d-4901-958a-591b94dbdae')).toThrowError(
      InvalidCVIdError,
    ); // Invalid length: 32
    expect(() => new CvId('944fadf6603d4901-958a-591b94dbdaec')).toThrowError(
      InvalidCVIdError,
    ); // Missing a separator
    expect(() => new CvId('w44fadf6603d4901-958a-591b94dbdaec')).toThrowError(
      InvalidCVIdError,
    ); // Invalid hex-character: w
  });
