...
describe('Meeting identity', () => {
  ...
  it('should fail when invalid id is used', () => {
    expect(() => new MeetingIDVO('asdaxxxdasfa')).toThrowError(
      InvalidMeetingIdError,
    ); // Non UUID string
    expect(
      () => new MeetingIDVO('666poxx6603d-7979-456a-876b21dbdae'),
    ).toThrowError(InvalidMeetingIdError); // Invalid length: 32
    expect(
      () => new MeetingIDVO('666poxx6603d4901-456a-876b21dbdaec'),
    ).toThrowError(InvalidMeetingIdError); // Missing a separator
    expect(
      () => new MeetingIDVO('696poxx6603d4901-456a-876b21dbdaec'),
    ).toThrowError(InvalidMeetingIdError); // Invalid hex-character: w
  });
