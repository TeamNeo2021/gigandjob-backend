  ...
  async Handle(command: any): Promise<void> {
    switch (command.constructor) {
      case CandidateRegisterDTO: {
        const dto: CandidateRegisterDTO = <CandidateRegisterDTO>command;
      ...
      case SuspendCandidateDTO: {
        ...
        if (
          candidate.state.suspensionCount + 1 >=
          (await this.configuration.getSuspensionLimit())
        ) {
          candidate.eliminateThisCandidate();
          await this.repository.eliminate(candidate.id);
        } else {
          candidate.suspendThisCandidate();
          await this.scheduler.scheduleCandidateReactivation(
            candidate.id,
            until,
          );
          await this.repository.save(candidate);
