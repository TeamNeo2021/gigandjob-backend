...
export class EmployerApplicationService {
  ...
  async Handle(command: any): Promise<void> {
    switch (command.constructor) {
      ...
      case ReactivateEmployerDTO: {
        const cmd: ReactivateEmployerDTO = <ReactivateEmployerDTO>command;
        const employerReactivated = await this.repository.get(cmd.id);

        if (!employerReactivated) throw new Error(); //CouldNotFindEmployerError(employer.employerId)

        const newEmployerReactivated =
          EntitiesFactory.fromEmployerDtoToEmployer(employerReactivated);
        newEmployerReactivated.reactivateThisEmployer();
        await this.repository.save(
          EntitiesFactory.fromEmployerToEmployerDTO(newEmployerReactivated),
        );
        break;
      }
      ...
      case CreateEmployerCommandDTO: {
        ...
        await this.repository.save(
          EntitiesFactory.fromEmployerToEmployerDTO(employer),
        );
        break;
      }
    }
