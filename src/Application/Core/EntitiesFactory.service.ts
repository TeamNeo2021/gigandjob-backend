...
import {
  EmployerStates,
  EmployerStateVO,
} from 'src/Dominio/AggRoots/Employer/ValueObjects/EmployerStateVo';
...
export class EntitiesFactory {
  ...
  static fromCandidateDtoToCanditate(candidateDto: CandidateDTO): Candidate {
    const candidate: Candidate = new Candidate(
      ...
      new CandidateFullNameVo(
        candidateDto.name.split(' ')[0],
        candidateDto.name.split(' ')[1],
      ),
      new CandidatePhoneVo(
        candidateDto.phone.slice(0, 4),
        candidateDto.phone.slice(5),
      ),
      new CandidateEmailVo(candidateDto.email),
      new CandidateBirthDateVo(candidateDto.birthDate),
      new CandidateLocationVo(
        candidateDto.location.latitude,
        candidateDto.location.longitude,
      ),
    );
    return candidate;
  }
  ...
  static fromOfferDTOtoOffer(OfferDTO: OfferDTO): Offer {
    const offer: Offer = new Offer(
      ...
      new OfferLocationVO(
        OfferDTO.Direction.latitude,
        OfferDTO.Direction.longitude,
      ),
  ...
  static fromCreateOfferDTOtoOffer(OfferDTO: createOfferDTO): Offer {
    //PILAS ESTO ES PARA CREAR UN OFFER
  ...
  static fromOfferToOfferDTO(offer: Offer): OfferDTO {
    const newOfferDto: OfferDTO = new OfferDTO({
      ...
      applications: offer._application.map(
        (application) =>
          new ApplicationDTO({
