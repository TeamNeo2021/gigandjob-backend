...
const eliminateService = new CandidateApplicationService(
  memoryRepo,
  {
...
describe('eliminate a candidate in memory', () => {
  it('should suceed when eliminating a valid Candidate', () => {
    ...
    expect(
      eliminateService.Handle(
        new EliminateCandidateDTO(mockCandidate.Id.value),
      ),
    );
  });
});
