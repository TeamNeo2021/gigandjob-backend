...
class mockedOfferRepo implements IOfferRepository {
  ...
  async save(offer: OfferDTO): Promise<void> {
    const _offer = Offer.CreateOffer(
  ...
  async getOfferById(id: string): Promise<OfferDTO> {
    const returned_offer: OfferDTO = new OfferDTO({
      OfferId: id,
      ...
      applications: this.mockedApplications.map(
        (application) =>
          new ApplicationDTO({
...
describe('Offer application service', () => {
  ...
  it('should fail when command doesnt exist', async () => {
    const actualService: OfferApplicationService = create_offer_service();
  ...
  it('should save the new offer in DB', async () => {
    const actualService: OfferApplicationService = create_offer_service();
    const previousDB_length: number = mockedDB.length;
