...
describe('Eliminar una oferta', () => {
  it('debe tener éxito al suspender una oferta cuando esta activa', async () => {
    ...
    const exampleOffer: OfferDTO = await Orepo.getOfferById(offerDTO.OfferId);
    const offer = EntitiesFactory.fromOfferDTOtoOffer(exampleOffer);
    const ExCommand = new EliminatedOfferDTO((await offer)._Id.value);
    const OfferService = create_Service(Orepo);
    await OfferService.Handle(ExCommand);
    const EliminatedOffer: OfferDTO = await Orepo.getOfferById(
      offer._Id._value,
    );
    expect(
      EliminatedOffer.State.toString() == OfferStatesEnum.Eliminated.toString(),
    );
  });
  ...
  it('no debe tener éxito cuando una oferta esta cerrada', async () => {
    ...
    const ExCommand = new EliminatedOfferDTO((await offer)._Id.value);
    const OfferService = create_Service(Orepo);
    expect(OfferService.Handle(ExCommand)).rejects.toThrowError(
      InvalidOfferState,
    );
  });
});
